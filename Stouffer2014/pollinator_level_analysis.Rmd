...
for (network_path in networks_paths)
{
  ...
  {
    network = process_network(paste(networks_dir,network_path, sep=""))
    network_pollinator_features = get_pollinator_features(network, species_classification_path)
...
pollinator_features = read_csv("./data/pollinator_species_features.csv", show_col_types = FALSE)
```
