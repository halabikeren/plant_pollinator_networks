...
```{r}
networks_dir = "G:/My Drive/PhD/plant_pollinator_inter/Stouffer2014/data/raw_CSVs/networks/"
...
x <- foreach(i = 1:length(networks_paths)) %dopar% {
  network_path = networks_paths[i]
...
write.csv(species_features, "./data/plant_species_features.csv", row.names = TRUE)
```



```{r}
species_features = read_csv("./data/plant_species_features.csv", show_col_types = FALSE)
