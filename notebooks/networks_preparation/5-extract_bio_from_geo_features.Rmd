...
Observations = read.csv("../../data/networks/all/networks_metadata.csv")
...
Ecoregion_Polygon = rbind(rbind(Ecoregion_Polygon_1[, commonCols], Ecoregion_Polygon_2[, commonCols]), Ecoregion_Polygon_3[, commonCols])
```
...
Observation_SF_3 = st_transform(Observation_SF,
                          crs = st_crs(Ecoregion_Polygon_3))
...
sf_use_s2(FALSE)
Intersection_Data_1 = as.data.frame(st_intersects(Observation_SF_1, Ecoregion_Polygon_1))
Intersection_Data_2 = as.data.frame(st_intersects(Observation_SF_2, Ecoregion_Polygon_2))
Intersection_Data_3 = as.data.frame(st_intersects(Observation_SF_3, Ecoregion_Polygon_3))

#Convert the row.ID to the observation ID
Intersection_Data_1$Obs_ID = Observations$ID[Intersection_Data_1$row.id]
Intersection_Data_2$Obs_ID = Observations$ID[Intersection_Data_2$row.id]
Intersection_Data_3$Obs_ID = Observations$ID[Intersection_Data_3$row.id]

#Convert the col.ID to the ecoregion ID
Intersection_Data_1$EcoRegion = Ecoregion_Polygon_1$ECO_ID[Intersection_Data_1$col.id]
Intersection_Data_2$EcoRegion = Ecoregion_Polygon_2$ECO_ID[Intersection_Data_2$col.id]
Intersection_Data_3$EcoRegion = Ecoregion_Polygon_3$ECO_ID[Intersection_Data_3$col.id]
Intersection_Data_1$EcoRegion_name = Ecoregion_Polygon_1$ECO_NAME[Intersection_Data_1$col.id]
Intersection_Data_2$EcoRegion_name = Ecoregion_Polygon_2$ECO_NAME[Intersection_Data_2$col.id]
Intersection_Data_3$EcoRegion_name = Ecoregion_Polygon_3$ECO_NAME[Intersection_Data_3$col.id]
```
...
#Create a holder
BioClim_data_1 = as.data.frame(matrix(nrow = nrow(Observations),
                                    ncol = length(BioClim_Rasters) +1))
BioClim_data_2 = as.data.frame(matrix(nrow = nrow(Observations),
                                    ncol = length(BioClim_Rasters) +1))
BioClim_data_3 = as.data.frame(matrix(nrow = nrow(Observations),
                                    ncol = length(BioClim_Rasters) +1))

#Set the column names
colnames(BioClim_data_1) = c("Obs_ID",
                            paste("BioClim_",
                                1:19,
                                sep = ""))
colnames(BioClim_data_2) = c("Obs_ID",
                            paste("BioClim_",
                                1:19,
                                sep = ""))
colnames(BioClim_data_3) = c("Obs_ID",
                            paste("BioClim_",
                                1:19,
                                sep = ""))
  
#Set the observation ID
BioClim_data_1$Obs_ID = Observations$ID
BioClim_data_2$Obs_ID = Observations$ID
BioClim_data_3$Obs_ID = Observations$ID
```
...
for (i in 1:length(BioClim_Rasters)) 
{
  ...
  BioClim_data_1[, i+1] = raster::extract(biovar, Observation_SF_1)
  BioClim_data_2[, i+1] = raster::extract(biovar, Observation_SF_2)
  BioClim_data_3[, i+1] = raster::extract(biovar, Observation_SF_3)
}
```
...
Complete_Data_1 = merge(Intersection_Data_1[, c("Obs_ID", "EcoRegion", "EcoRegion_name")], 
                      BioClim_data_1,
                      by.x = "Obs_ID",
                      by.y = "Obs_ID")

Complete_Data_2 = merge(Intersection_Data_2[, c("Obs_ID", "EcoRegion", "EcoRegion_name")], 
                      BioClim_data_2,
                      by.x = "Obs_ID",
                      by.y = "Obs_ID")
Complete_Data_3 = merge(Intersection_Data_3[, c("Obs_ID", "EcoRegion", "EcoRegion_name")], 
                      BioClim_data_2,
                      by.x = "Obs_ID",
                      by.y = "Obs_ID")
```
...
#Add the data to the Intersection_Data for those whose ID match
Richness_Complete_Data_1 = merge(Intersection_Data_1[, c("Obs_ID", "EcoRegion")], 
                      richness_data,
                      by.x = "Obs_ID",
                      by.y = "Obs_ID")

Richness_Complete_Data_2 = merge(Intersection_Data_2[, c("Obs_ID", "EcoRegion")], 
                      richness_data,
                      by.x = "Obs_ID",
                      by.y = "Obs_ID")

Richness_Complete_Data_3 = merge(Intersection_Data_3[, c("Obs_ID", "EcoRegion")], 
                      richness_data,
                      by.x = "Obs_ID",
                      by.y = "Obs_ID")

Richness_Complete_Data_1 <- Richness_Complete_Data_1[c("Obs_ID", "EcoRegion", "species_richness")]

Richness_Complete_Data_2 <- Richness_Complete_Data_2[c("Obs_ID", "EcoRegion", "species_richness")]

Richness_Complete_Data_3 <- Richness_Complete_Data_3[c("Obs_ID", "EcoRegion", "species_richness")]
```

```{r}
full_data_1 = merge(Complete_Data_1, 
                  Richness_Complete_Data_1,
                  by.x = "Obs_ID",
                  by.y = "Obs_ID")
full_data_2 = merge(Complete_Data_2, 
                  Richness_Complete_Data_2,
                  by.x = "Obs_ID",
                  by.y = "Obs_ID")
full_data_3 = merge(Complete_Data_3, 
                  Richness_Complete_Data_3,
                  by.x = "Obs_ID",
                  by.y = "Obs_ID")

uni_data = rbind(full_data_1, full_data_2, full_data_3)
```
